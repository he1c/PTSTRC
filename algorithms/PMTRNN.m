function Xhat=PMTRNN(MissM,Mask,I,option)
...
for t=1:1:total_frame_num
    ...
    if t==1
        ...
        MissMinterval=MissMse(1:interval:end,1:interval:end,:);
        MissMstack=uint8(zeros(size(MissMinterval,1),size(MissMinterval,2),3,interval^2));
        Maskstack=false(size(MissMstack));
        for i=1:interval^2   
            si=sm(i);sj=sn(i);
            cc=mod(si-1,interval)+mod(sj-1,interval)*interval+1;
            MissMstack(:,:,:,cc)=MissMse(si:interval:end,sj:interval:end,:);    
            Maskstack(:,:,:,cc)=Maskse(si:interval:end,sj:interval:end,:); 
        end
        patchnew=gpuArray(image2patch(MissMstack, dims));
        patchmasknew=gpuArray(image2patch(Maskstack, dims));
        else
        patch_ind = ind_new;
    end
    ...
    for i =1:length(patch_ind_s)        
       if patch_ind_s(i)~=0
           ...
           refmask = patchmaskold(:,patch_ind_s(i),patch_ind_c(i));
           if sum(refmask(:))>min_ob_num
               [blk_patch_s,error_arr_s] = patch_BM_adjacent_frame_stack(patchnew, patchmasknew, refpatch, refmask, total_patch_size_s, param, patch_ind_s(i)); 
    ...
    for i =1:length(ind_add_s)        
       ...
       refmask = patchmasknew(:,ind_add_s(i),ind_add_c(i));
       [blk_patch_s,error_arr_s] = patch_BM_adjacent_frame_stack(patchnew, patchmasknew, refpatch, refmask, total_patch_size_s, param, ind_add_s(i)); 
       for s=1:1:interval^2     
            blk_patch((s-1)*p_frame+1:s*p_frame,i)=blk_arr_part{s}(blk_patch_s(:,s));
            err_patch((s-1)*p_frame+1:s*p_frame,i)=error_arr_s(:,s);
       end
    ...
    parfor kk=1:length(ind_new)
        if ind_new(kk)~=0
            ...
            if ~isempty(Utr_TR{kk})
                ...
                Utr_TR{kk} = TRSSD_update(patch_miss, double(patch_miss~=-1), para_TR, Utr_TR{kk}, Ko); 
            else 
                Utr_TR{kk} = TRNN(patch_miss, double(patch_miss~=-1), [], para_TRNN);     
            end       
        end
    end   
    ...
    for kk=1:1:length(blk_arr_out_ind)
        ...
        patch_hat=Utr_TR{blk_arr_out_ind(kk)};
