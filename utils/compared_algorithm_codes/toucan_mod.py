...
def toucan(Y,mask,dim):
    ...
    Y=np.reshape(Y, (dim[0], dim[1], dim[2]));
    mask=np.reshape(mask, (dim[0], dim[1], dim[2]));

    # print('pixel: {:.8f} {:.8f} {:.8f} {:.8f}'.format(Y[0,0,0],Y[1,0,0],Y[2,0,0],Y[3,0,0]))

    Y=Tensor(np.transpose(Y,[0,2,1]));
    mask=Tensor(np.transpose(mask,[0,2,1]));
    ...
    for outer in range(0, outer):
        ...
        for inner in range(0, n2):
            ...
            tStart = time.time()
            U, w, cg_iters = toucan_stream(Yvec, mask.array()[:, frame_idx, :], U, step, cgtol=cgtol)
            tEnd = time.time()
            ...
            fun = lambda X,k: [0, tfrobnorm_array(X.array()[:,k,:] * mask.array()[:,k,:]- Y.array()[:,k,:]) / tfrobnorm_array(Y.array()[:,k,:])];
            ...
            # print('NRMSE: {:.8f} '.format(nrmse))


    tElapsed = np.sum(stats_toucan[:,-1])


    return Lhat, U, stats_toucan, tElapsed
