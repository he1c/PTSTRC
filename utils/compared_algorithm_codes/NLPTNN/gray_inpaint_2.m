function img_out = gray_inpaint_2(img, mask, radius, search_rad, search_gap, inpaint_algo, para, THRESHOLD)
% mask --- is_missing_mat

[row,col] = size(img);

x(1) = radius+1; x(2) = row-radius;
x_range = x(1):2*radius:x(2); x_range_len = length(x_range);
if x_range(x_range_len) ~= x(2)
    x_range = [x_range,x(2)];
    x_range_len = x_range_len + 1;
end
y(1) = radius+1; y(2) = col-radius;
y_range = y(1):2*radius:y(2); y_range_len = length(y_range);
...
parfor k = 1:all_points_num
    ...
    s_init = point2roi(all_points(k,:),img_pre,radius);
    index_set = patch_group(all_points(k,:), radius, search_rad, search_gap, img_pre, THRESHOLD, 2); % mode 2
    ...
    for j = 1:THRESHOLD
        index_img = point2roi(index_set(j,:),img,radius);
        index_mask = point2roi(index_set(j,:),mask,radius);
        index_init = point2roi(index_set(j,:),img_pre,radius);
        g_tensor(:,:,j+1) = index_img;
        g_mask(:,:,j+1) = index_mask;
        g_init(:,:,j+1) = index_init;
    end
    g_tensor_out = inpaint_algo(g_tensor, g_init, g_mask, para);
