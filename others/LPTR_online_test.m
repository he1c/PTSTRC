function Xhat=LPTR_online_test(MissM,Mask,I,option)
...
if buffersize>total_frame_num
    error('buffersize shoule be less than total frame number');
end
MissM=cat(4,MissM,MissM(:,:,:,end-buffersize+1:end-1));
Mask=cat(4,Mask,Mask(:,:,:,end-buffersize+1:end-1));
I=cat(4,I,I(:,:,:,end-buffersize+1:end-1));
MissM=squeeze(MissM);
Mask=squeeze(Mask);
I=squeeze(I);
...
AA=cell(1,length(ind_initial));
Ax=cell(1,length(ind_initial));
...
%     MissM_t = padarray(MissM_t,[border_ext border_ext],'symmetric','both');
%     Mask_t = padarray(Mask_t,[border_ext border_ext],'symmetric','both');
    ...
    I_t = padarray(I_t,[border_ext_col_up border_ext_row_left],'symmetric','pre');
    I_t = padarray(I_t,[border_ext_col_down border_ext_row_right],'symmetric','post');
    ...
    if t>buffersize
        ...
        AA=AA(ind_zero);
        Ax=Ax(ind_zero);
        ind_zero=sum(squeeze(blk_arr(:,:,1)))~=0;
        ...
        %ind_new=ind_new-patch_ind_adjust.*(ind_new~=0);
        % add new entry
        Buffer{buffersize}=MissMdouble;
        BufferI{buffersize}=Idouble;
        blk_arr(buffersize,:,:)=0;
    else
        Buffer{t}=MissMdouble;
        BufferI{t}=Idouble;
    end
    ...
    AA_pre=AA;
    AA=cell(1,length(ind_new));
    AA(1:length(AA_pre))=AA_pre;
    
    Ax_pre=Ax;
    Ax=cell(1,length(ind_new));
    Ax(1:length(Ax_pre))=Ax_pre;
    ...
    if t>3
        for kk=1:length(ind_new)
            if ind_new(kk)~=0
                ...
                arr=arr(:);               
                n_p=min(p_frame*buffersize,sum(arr~=0));
                if n_p>=batch_start_size
                    if nChannel==1
                        patch_miss=zeros(dim,dim,n_p);  
                        patch_full=patch_miss;  
                    else
                    ...
                    for i=1:1:length(arr)
                        ...
                        yyn=yy:min(yy+dim-1,n);
                        if nChannel==1
                            patch_miss(1:length(xxn),1:length(yyn),p_count)=Buffer{zz}(xxn,yyn);
                            patch_full(1:length(xxn),1:length(yyn),p_count)=BufferI{zz}(xxn,yyn);
                        elseif nChannel==3
                            patch_miss(1:length(xxn),1:length(yyn),:,p_count)=Buffer{zz}(xxn,yyn,:);
                            patch_full(1:length(xxn),1:length(yyn),:,p_count)=BufferI{zz}(xxn,yyn,:);
                        end
                    ...
                    if ~isempty(Utr_TR{kk})%n_p>batch_start_size
                        %Utr_TR{kk} = Completion_TR_add(patch_miss, double(patch_miss~=-1), para_TR, Utr_TR{kk}, p_frame, p_frame*buffersize);   
                        [Utr_TR{kk}, AA{kk}, Ax{kk}] = TRRLS_add(patch_miss(:,:,:,end-p_frame+1:end), double(patch_miss(:,:,:,end-p_frame+1:end)~=-1), para_TR, Utr_TR{kk}, AA{kk}, Ax{kk});
                    else 
                        [Utr_TR{kk}, AA{kk}, Ax{kk}] = TRRLS_init(patch_miss, double(patch_miss~=-1), para_TR); 
    ...
    if t-buffersize+1>0
        ...
        blk_arr_out=squeeze(blk_arr(1,:,1));
        
        blk_arr_out_ind=find(blk_arr_out~=0);
        
        blk_acc=sum(blk_arr(1:end,blk_arr_out_ind,1)~=0);
        
        for kk=1:1:length(blk_arr_out_ind)
            
            Utr_n=Utr_TR{blk_arr_out_ind(kk)};
            ind_utr=buffersize-blk_acc(kk)+1;
            Utr_n{end}=Utr_n{end}(:,(ind_utr-1)*p_frame+1:ind_utr*p_frame,:);          
            patch_hat=reshape(fullTR(Utr_n),dim,dim,nChannel,p_frame);  
            for ii=1:1:p_frame
                [xx,yy,~]=ind2sub(total_buffer_size,blk_arr(1,blk_arr_out_ind(kk),ii));
                xxn=xx:min(xx+dim-1,m);
                yyn=yy:min(yy+dim-1,n);
        ...
        Xhat(:,:,:,t-buffersize+1)=I_hat(border_ext_col_up+1:end-border_ext_col_down,border_ext_row_left+1:end-border_ext_row_right,:)./C_hat(border_ext_col_up+1:end-border_ext_col_down,border_ext_row_left+1:end-border_ext_row_right);
        disp(psnr(I(:,:,:,t-buffersize+1),Xhat(:,:,:,t-buffersize+1)));
