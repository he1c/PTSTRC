function Xhat=OPTRC_buffer(MissM,Mask,I,option)
    ...
    for kk=1:length(ind_new)
        if ind_new(kk)~=0
            ...
            if ~isempty(Utr_TR{kk})
                Utr_TR_pre = Utr_TR{kk}{end}(:,end-p_frame+1:end,:);
                Utr_TR{kk} = TRSGD_scaled_add(patch_miss(:,:,:,end-p_frame+1:end), double(patch_miss(:,:,:,end-p_frame+1:end)~=-1), para_TR, Utr_TR{kk}, p_frame); 
                Utr_TR{kk}{end}=cat(2,Utr_TR_pre,Utr_TR{kk}{end});
            else 
