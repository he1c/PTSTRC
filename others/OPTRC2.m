function Xhat=OPTRC2(MissM,Mask,I,option)
...
%% initial variables
C=zeros(m,n);
blk_arr=zeros(buffersize,length(ind_initial),p_frame*2);
...
for t=1:1:total_frame_num
    ...
    if t==1
        ...
        p_frame=2*option.p_frame;
        param.K=p_frame;
    else
        patch_ind = ind_new;
        p_frame=option.p_frame;
        param.K=p_frame;
        blk_arr=blk_arr(:,:,1:p_frame);
    end
    ...
    for i =1:length(patch_ind_s)        
       if patch_ind_s(i)~=0
           ...
           for s=1:1:interval^2                 
                [blk_patch_s,error_arr_s] = patch_BM_adjacent_frame_weighted(patchnew{s}, patchmasknew{s}, refpatch, refmask, total_patch_size_s, param, patch_ind_s(i)); 
    ...
    for i =1:length(ind_add_s)        
       ...
       for s=1:1:interval^2                 
            [blk_patch_s,error_arr_s] = patch_BM_adjacent_frame_weighted(patchnew{s}, patchmasknew{s}, refpatch, refmask, total_patch_size_s, param, ind_add_s(i)); 
            blk_patch((s-1)*p_frame+1:s*p_frame,i)=blk_arr_part{s}(blk_patch_s);
            err_patch((s-1)*p_frame+1:s*p_frame,i)=error_arr_s;
       end
    end
    ...
    parfor kk=1:length(ind_new)
        if ind_new(kk)~=0
            ...
            if ~isempty(Utr_TR{kk})
                Utr_TR{kk} = TRSGD_scaled_add(patch_miss, double(patch_miss~=-1), Utr_TR{kk}, p_frame); 
            else 
