function [Xhat,timecost]=OPTRC_SGD(MissM,Mask,I,option)
    ...
    parfor kk=1:length(ind_new)
        if ind_new(kk)~=0
            ...
            if ~isempty(Utr_TR{kk})
                ...
                Utr_TR{kk} = TRSGD_add(patch_miss, double(patch_miss~=-1), Utr_TR{kk}, Ko); 
            else 
                Utr_TR{kk} = TRSGD_initial(patch_miss, double(patch_miss~=-1), para_TR);        
            end       
        end
    end   
