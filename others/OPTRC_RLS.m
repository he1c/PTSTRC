function [Xhat,timecost]=OPTRC_RLS(MissM,Mask,I,option)
...
for t=1:1:total_frame_num
    ...
    for kk=1:length(ind_new)
        if ind_new(kk)~=0
            ...
            if ~isempty(Utr_TR{kk})
                [Utr_TR{kk}, AA{kk}, Ax{kk}] = TRRLS_add(patch_miss, double(patch_miss~=-1), para_TR, Utr_TR{kk}, AA{kk}, Ax{kk}, p_frame);
            else 
    ...
    fprintf('%3i, %.4fs, %5i', t, timecost(end), length(blk_arr_out_ind));     
