function Xhat=OPTRC_block3(MissM,Mask,I,option)
...
for t=1:1:total_frame_num
    ...
    if t==1
        ...
        for i=1:interval^2   
            ...
            patchmaskorinew{cc} = image2patch(Mask_t(si:interval:end,sj:interval:end,:), dims); 
        end
    else
        patch_ind = ind_new;
    end
...
%             if t>1&&min(min(C_pre(xxn,yyn)))>2&&sum(squeeze(blk_arr_pre(:,i,1))~=0)==buffersize
%                continue;
%             end
...
%     imshow(BW);
%     hold on
%     for i=1:1:length(ind_add)
%         [xxn,yyn,zzn]=ind2sub(total_patch_size,ind_add(i));
%         rectangle('Position',[yyn xxn dim dim],'EdgeColor','g');
%     end
