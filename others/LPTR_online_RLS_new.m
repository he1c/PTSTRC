function Xhat=LPTR_online_RLS_new(MissM,Mask,I,option)
...
%     Mask_t = padarray(Mask_t,[border_ext border_ext],'symmetric','both');
        ...
        parfor kk=1:length(ind_new)
            if ind_new(kk)~=0
                ...
                if ~isempty(Utr_TR{kk})
                    [Utr_TR{kk}, AA{kk}, Ax{kk}] = TRRLS_add(patch_miss(:,:,:,end-p_frame+1:end), double(patch_miss(:,:,:,end-p_frame+1:end)~=-1), para_TR, Utr_TR{kk}, AA{kk}, Ax{kk}, p_frame);
                else 
    ...
    if t-buffersize+1>0
        ...
        blk_acc=sum(blk_arr(1:end,blk_arr_out_ind,1)~=0,1);
