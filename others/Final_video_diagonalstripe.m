...
for i=1:1:frame_num
    ...
    Mask_k = uint8(ones(n_o(2)*2,n_o(2)*2,3));
    omega=mod(i,stripe_interval)+1:stripe_interval:n_o(2)*2;
    for j=1:1:n_o(2)*2
        if ismember(j,omega)
            for cc=1:1:j
                Mask_k(j-cc+1,cc,:)=0;
    ...
    Mask=cat(4,Mask,Mask_k(1:n_o(1),1:n_o(2),:,:));
...
% %% PTRC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% opt.n_t=[3 4 4 6 2 4 4 11 3 5 10];
% opt.eta=0.1;
